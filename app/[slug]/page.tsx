...
function getCover(keyword: string | undefined, slug: string | undefined): { gradient: string; emoji: string } {
  ...
  if (k.includes('paraphraser') || k.includes('story') || k.includes('writ')) return { gradient: 'linear-gradient(135deg,#4f8bff,#7c5cfc)', emoji: '✍️' }
...
export default async function ArticlePage({ params }: Props) {
  ...
  // Related articles — computed before JSX
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .related-card-title { font-size:0.9rem; font-weight:700; line-height:1.4; color:var(--text); flex:1; }
        ...
        .comment-name-input,.comment-body-input { width:100%; padding:10px 14px; background:var(--bg); border:1px solid var(--border); border-radius:8px; color:var(--text); font-size:0.9rem; outline:none; }
